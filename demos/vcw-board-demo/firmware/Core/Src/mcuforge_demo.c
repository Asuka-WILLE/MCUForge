...
#define MCUFORGE_LINK_LOSS_TIMEOUT_MS 150U
#define MCUFORGE_RECOVERY_NEUTRAL_FRAMES 3U

typedef enum
{
    MCUFORGE_STATE_DISABLED = 0,
    MCUFORGE_STATE_RUN,
    MCUFORGE_STATE_FAILSAFE,
    MCUFORGE_STATE_ESTOP
} MCUForgeState;
...
typedef struct
{
    ...
    MCUForgeState state;
    uint8_t recovery_neutral_count;
} MCUForgeDemoState;
...
static const char *mcuforge_state_name(MCUForgeState state)
{
    switch(state)
    {
        case MCUFORGE_STATE_RUN:
            return "RUN";
        case MCUFORGE_STATE_FAILSAFE:
            return "FAILSAFE";
        case MCUFORGE_STATE_ESTOP:
            return "ESTOP";
        case MCUFORGE_STATE_DISABLED:
        default:
            return "DISABLED";
    }
}

static void mcuforge_zero_outputs(void)
{
...
static void mcuforge_accept_frame(const uint8_t frame[MCUFORGE_CONTROL_FRAME_SIZE], uint32_t now)
{
    ...
    uint8_t enabled = ((flags & MCUFORGE_FLAG_ENABLE) != 0U) ? 1U : 0U;
    uint8_t estop = ((flags & MCUFORGE_FLAG_ESTOP) != 0U) ? 1U : 0U;

    demo_state.sequence = mcuforge_read_u16_le(&frame[4]);
    demo_state.throttle = throttle;
    demo_state.steering = steering;
    demo_state.enabled = enabled;
    demo_state.emergency_stop_requested = estop;
    demo_state.frame_seen = 1U;
    demo_state.last_valid_tick = now;
    demo_state.valid_frame_count++;

    /*
     * ESTOP latches until the demo is re-initialised (MCU reset). Once a valid
     * frame requests an emergency stop the outputs are cleared and no later
     * frame, link loss, or recovery sequence can un-latch it.
     */
    if(estop || demo_state.state == MCUFORGE_STATE_ESTOP)
    {
        demo_state.state = MCUFORGE_STATE_ESTOP;
        demo_state.recovery_neutral_count = 0U;
        mcuforge_zero_outputs();
        return;
    }

    if(demo_state.state == MCUFORGE_STATE_FAILSAFE)
    {
        /*
         * Controlled recovery: three consecutive neutral frames
         * (throttle == 0 && steering == 0) are required before outputs may be
         * re-armed. Any non-neutral frame restarts the count. The qualifying
         * neutral frame keeps the outputs at zero.
         */
        if(throttle == 0 && steering == 0)
        {
            demo_state.recovery_neutral_count++;
            if(demo_state.recovery_neutral_count >= MCUFORGE_RECOVERY_NEUTRAL_FRAMES)
            {
                demo_state.state = enabled ? MCUFORGE_STATE_RUN : MCUFORGE_STATE_DISABLED;
            }
            mcuforge_zero_outputs();
        }
        else
...
static void mcuforge_check_link_loss(uint32_t now)
{
    /*
     * Link-loss FAILSAFE: when no valid frame has arrived within the timeout,
     * the demo clears its outputs and enters FAILSAFE. ESTOP is a latched state
     * and FAILSAFE is already inside its recovery gate, so neither is
     * re-triggered here.
     */
    if(demo_state.state != MCUFORGE_STATE_RUN &&
       demo_state.state != MCUFORGE_STATE_DISABLED)
    {
        return;
    }

    if(demo_state.frame_seen &&
       (now - demo_state.last_valid_tick) > MCUFORGE_LINK_LOSS_TIMEOUT_MS)
    {
        demo_state.state = MCUFORGE_STATE_FAILSAFE;
        demo_state.recovery_neutral_count = 0U;
        mcuforge_zero_outputs();
    }
}
...
static void mcuforge_lcd_process(uint32_t now)
{
    ...
    mcuforge_lcd_show_line(7U, YELLOW, "STATE:%s",
                          mcuforge_state_name(demo_state.state));
}
...
static void mcuforge_telemetry_process(uint32_t now)
{
    ...
    length = snprintf(line, sizeof(line),
                      ...
                      "\"state\":\"%s\",\"pc_recovery_neutral_count\":%u,\"pc_valid_frame_count\":%lu,"
                      ...
                      (unsigned int)(demo_state.state == MCUFORGE_STATE_RUN),
                      mcuforge_state_name(demo_state.state),
                      (unsigned int)demo_state.recovery_neutral_count,
                      (unsigned long)demo_state.valid_frame_count,
                      (unsigned long)demo_state.invalid_frame_count,
