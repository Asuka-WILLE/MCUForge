...
#define STRAIGHT_SYNC_LAUNCH_TRIM_GAIN       0.9f
#define STRAIGHT_SYNC_LAUNCH_FAST_RPM         4
#define STRAIGHT_SYNC_LAUNCH_LAGGING_RPM      8
...
static uint32_t left_feedback_sequence=0U;
static uint32_t right_feedback_sequence=0U;
...
static void straight_sync_apply(int16_t *left_cmd, int16_t *right_cmd, uint32_t now)
{
    ...
    if(straight_sync.launch_start_tick == 0U ||
       straight_sync.launch_direction != travel_direction)
    {
        straight_sync.launch_start_tick = now;
        straight_sync.launch_direction = travel_direction;
    }
    launch_window_active = ((now - straight_sync.launch_start_tick) <=
                            STRAIGHT_SYNC_LAUNCH_WINDOW_MS);

    feedback_fresh = ((now - left_feedback_tick) <= STRAIGHT_SYNC_FEEDBACK_MAX_AGE_MS &&
    ...
    /*
     * A speed comparison is valid only after both wheels have produced a new
    ...
    if(paired_update_ready || tail_update_ready)
    {
        ...
        straight_sync.pair_skew_ms = feedback_skew_ms;
        if(feedback_skew_ms > STRAIGHT_SYNC_PAIR_MAX_SKEW_MS)
        {
            straight_sync.filtered_error = 0.0f;
            straight_sync.trim = 0;
            return;
...
static void telemetry_send(void)
{
    ...
    int len = snprintf(line, sizeof(line),
                       ...
                       lift_height_mm,
                       (unsigned long)wheel_feedback_sequence,
                       (unsigned long)left_speed_age_ms,
                       (unsigned long)right_speed_age_ms,
