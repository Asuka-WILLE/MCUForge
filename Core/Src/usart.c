...
void MX_UART5_Init(void)
{
  ...
  /* SBUS is receive-only; PB13 is reserved for the onboard BMI088 SPI2 clock. */
...
HAL_StatusTypeDef RS485_SendPacket(uint8_t* pData, uint16_t len)
{
    ...
    HAL_StatusTypeDef sta = HAL_UART_Transmit(&huart2, pData, len, 100);
...
HAL_StatusTypeDef RS485_SendPacket2(uint8_t* pData, uint16_t len)
{
    ...
    // 发送数据
    HAL_StatusTypeDef sta_2 = HAL_UART_Transmit(&huart3, pData, len, 100);

    // 等待硬件发送完成（必须保留，这不是延时，是等硬件空）
    while (__HAL_UART_GET_FLAG(&huart3, UART_FLAG_TC) == RESET);

    // 立刻关闭发送 → 去掉 HAL_Delay(5)
...
HAL_StatusTypeDef RS485_ReceivePacket(uint8_t* pData, uint16_t len, uint32_t timeout)
{
    ...
    // 开始接收
    return HAL_UART_Receive(&huart2, pData, len, timeout);
}
...
HAL_StatusTypeDef RS485_ReceivePacket2(uint8_t* pData, uint16_t len, uint32_t timeout)
{
    ...
    // 开始接收
    return HAL_UART_Receive(&huart3, pData, len, timeout);
}


// 通用串口接收：有多少收多少，返回实际收到的字节数
int RS485_Receive_All(uint8_t *buf, uint8_t max_len, uint32_t timeout)
{
    uint8_t len = 0;
    uint32_t tick_start = HAL_GetTick();

    while (len < max_len && (HAL_GetTick() - tick_start) < timeout)
    {
        if (HAL_UART_Receive(&huart2, &buf[len], 1, 5) == HAL_OK)
        {
            len++;
            tick_start = HAL_GetTick(); // 收到就刷新超时
        }
    }
    return len;
}
