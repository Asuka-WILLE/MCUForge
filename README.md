...
本诊断版不改变 v1 的轨迹参数、左右轮目标值、右轮先发顺序和原有 `2 ms / 5 ms` 帧间隔，也不启用重试、`0x2309` 写入或停车尾段补偿。

- 每次向 `0x2318` 写速度后，在原有帧间隔内校验 Modbus `0x06` 应答的站号、寄存器、数值和 CRC；结果记录在 `*_write_echo_ok`、成功/失败计数、序号和值字段中。
- `0x500E` 高精度轮速按 40 ms 周期读取左右轮，只有一对均成功才更新 `speed_pair_sequence`；左右轮是同一采样对，但共享 RS485 总线不代表电气同时采样。
- 左右驱动器读帧之间保留 2 ms Modbus RTU 静默间隔；一次配对完成、失败或被控制命令中止后，周期从结束时重新计时，避免超时任务立即重发并占满总线。
- `0x5002～0x500A` 按 100 ms 周期成对读取，导出实时转矩、驱动器内部速度目标、运行模式和母线电压。
- `0x5015/0x5016` 按 100 ms 周期错开读取，导出累计反馈位置；升降遥测降为 500 ms。
- 启动后只读两台驱动器的软件版本、速度环、惯量/PID、加减速、零速和转矩限制参数，并记录支持位与左右不一致位；不会修改或保存任何驱动参数。
- 新增诊断量统一为车体物理方向：右轮速度、目标、转矩和累计位置已在固件中反号。旧 `left_rpm`、`right_rpm`、`left_cmd`、`right_cmd` 字段继续保留以兼容历史日志。

`PC_Tools/telemetry_monitor.py` 会优先用 `0x500E` 的 0.1 rpm 数据绘图，在界面底部显示写应答、内部目标、转矩和母线电压，并把全部诊断字段同步写入 CSV/JSONL。
...
14. 主循环末尾推进后台遥测状态机：旧 `0x5000` 反馈节奏保持不变，同时分时读取 40 ms 高精度轮速对、100 ms 驱动诊断对、100 ms 累计位置对和 500 ms 升降高度；任何控制写入均可中止后台读取。
